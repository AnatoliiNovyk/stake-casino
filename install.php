...
if (version_compare(phpversion(), '8.2', '<')) {
  $errors[] = sprintf('PHP <b>8.2</b> or higher should be installed (currently installed PHP %s).', phpversion());
}

foreach(['openssl','pdo','mbstring','tokenizer','xml','curl','ctype','fileinfo','json','bcmath','zip','gmp'] as $extension) {
  if (!extension_loaded($extension)) {
    $errors[] = sprintf('PHP <b>%s</b> extension should be installed and enabled.', $extension);
  }
}

foreach(['public', 'storage', 'storage/app/public', 'storage/framework', 'storage/framework/cache', 'storage/framework/sessions',
